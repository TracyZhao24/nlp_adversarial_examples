# Adopted code from https://github.com/Smerity/keras_snli
...
ACTIVATION = 'relu'
OPTIMIZER = 'rmsprop'
print('RNN / Embed / Sent = {}, {}, {}'.format(RNN,
                                               EMBED_HIDDEN_SIZE, SENT_HIDDEN_SIZE))
print('GloVe / Trainable Word Embeddings = {}, {}'.format(USE_GLOVE, TRAIN_EMBED))


def to_seq(X): return pad_sequences(
    tokenizer.texts_to_sequences(X), maxlen=MAX_LEN)
...
def build_model():
    ...
    if USE_GLOVE:
        ...
        embed = Embedding(VOCAB, EMBED_HIDDEN_SIZE, weights=[
            embedding_matrix], input_length=MAX_LEN, trainable=TRAIN_EMBED)
    else:
    ...
    rnn = SumEmbeddings if not RNN else RNN(
        return_sequences=False, **rnn_kwargs)
    prem = rnn(prem)
    hypo = rnn(hypo)
...
if __name__ == '__main__':
    ...
    loss, acc = model.evaluate(
        [test[0], test[1]], test[2], batch_size=BATCH_SIZE)
    print('Test loss / test accuracy = {:.4f} / {:.4f}'.format(loss, acc))
    # Save the model
    model.save_weights('nli_model.h5')
    with open('nli_tokenizer.pkl', 'wb') as fh:
        pickle.dump(tokenizer, fh)
    with open('nli_testing.pkl', 'wb') as fh:
        pickle.dump(test, fh)
